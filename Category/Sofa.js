...
const Sofa = ({ route, navigation }) => {
    const [category, setCategory] = useState([])

    useEffect(() => {
        const sofaItems = async () => {
            try {
              // fetch products in the sofa category
                const itemSofa = await FetchCategory('Sofa');
                setCategory(itemSofa);
            } catch (error) {
    ...
    <View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        ...
        <TouchableOpacity style={styles.buttonCategory} onPress={onPressLoginAdmin}>
          <Text style={styles.buttonTextCat}>Button7</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.buttonCategory} onPress={onPressLoginAdmin}>
          <Text style={styles.buttonTextCat}>Button8</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
